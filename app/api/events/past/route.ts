...
export async function GET(request: Request) {
    try {
        ...
        const backendUrl = `${BASE_URL}/events/past/${queryString ? `?${queryString}` : ''}`;

        console.log('Fetching past events from:', backendUrl);
        ...
        if (!response.ok) {
            ...
            return NextResponse.json(
                { error: 'Failed to fetch past events', detail: error },
        ...
        // The same sanitize function works because past events use the same structure!
