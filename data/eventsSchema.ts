...
export const ticketTypeSchema = Yup.object().shape({
    ...
    quantity: Yup.string()
        ...
        .test('max-value', 'Maximum quantity is 10,000', (value) => Number(value) <= 10000),
...
const recurrencePatternSchema = Yup.object().shape({
    ...
    interval: Yup.string()
        ...
        .test('is-number', 'Must be a valid number', (value) => !isNaN(Number(value)))
        .test('is-integer', 'Must be a whole number', (value) => Number.isInteger(Number(value)))
        .test('min-value', 'Interval must be at least 1', (value) => Number(value) >= 1)
        .test('max-value', 'Interval cannot exceed 30', (value) => Number(value) <= 30),
    end_date: Yup.string()
        .nullable(),
    occurrences: Yup.string()
        ...
        .test('is-integer', 'Must be a whole number', (value) => {
            if (!value) return true;
            return Number.isInteger(Number(value));
        })
        .test('min-value', 'Occurrences must be at least 2', (value) => {
            if (!value) return true;
            return Number(value) >= 2;
        })
        .test('max-value', 'Occurrences cannot exceed 365', (value) => {
            if (!value) return true;
